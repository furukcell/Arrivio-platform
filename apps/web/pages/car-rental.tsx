...
import { createCarRentalRequest, listPublicCarRentalDailyPrices } from "@arrivio/firebase";
import type { CarRentalDailyPrice } from "@arrivio/shared";
import {
...
export default function CarRentalPage() {
  ...
  const [dailyPrices, setDailyPrices] = useState<CarRentalDailyPrice[]>([]);
  const [status, setStatus] = useState("");
  const [requestCode, setRequestCode] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const livePrices = dailyPrices.length ? dailyPrices : undefined;
  const priceSummary = getCarRentalPriceSummary(form, livePrices);
  ...
  useEffect(() => {
    ...
    async function loadPrices() {
      try {
        const prices = await listPublicCarRentalDailyPrices();
        if (isMounted) setDailyPrices(prices);
      } catch (error) {
        if (isMounted) setDailyPrices([]);
      }
    }
    loadPrices();
  ...
  async function submitRequest() {
    ...
    try {
      ...
      await createCarRentalRequest(mapRentalFormToRequest(form, code, qrSourceId || undefined, language, dailyPrices));
      setRequestCode(code);
  ...
  return createElement("main", { style: pageStyle },
    createElement("section", { style: cardStyle },
      ...
      createElement("div", { style: gridStyle },
        ...
        createElement("label", null, text.pickupLocation, createElement("select", { style: inputStyle, value: form.pickupLocation, onChange: (event) => updateField("pickupLocation", event.currentTarget.value) }, ...CAR_RENTAL_PICKUP_OPTIONS.map((option) => createElement("option", { key: option.value, value: option.value }, language === "tr" ? option.trLabel : option.enLabel)))),
        createElement("label", null, language === "tr" ? "Bırakış yeri" : "Dropoff location", createElement("input", { style: inputStyle, value: form.dropoffLocation, onChange: (event) => updateField("dropoffLocation", event.currentTarget.value), placeholder: language === "tr" ? "Aynı yer veya otel" : "Same place or hotel" })),
        createElement("label", null, text.pickupDate, createElement("input", { style: inputStyle, type: "date", value: form.pickupDate, onChange: (event) => updateField("pickupDate", event.currentTarget.value) })),
        createElement("label", null, language === "tr" ? "Alış saati" : "Pickup time", createElement("input", { style: inputStyle, type: "time", value: form.pickupTime, onChange: (event) => updateField("pickupTime", event.currentTarget.value) })),
        createElement("label", null, text.dropoffDate, createElement("input", { style: inputStyle, type: "date", value: form.dropoffDate, onChange: (event) => updateField("dropoffDate", event.currentTarget.value) })),
        createElement("label", null, language === "tr" ? "Bırakış saati" : "Dropoff time", createElement("input", { style: inputStyle, type: "time", value: form.dropoffTime, onChange: (event) => updateField("dropoffTime", event.currentTarget.value) })),
        createElement("label", null, text.carClass, createElement("select", { style: inputStyle, value: form.carClass, onChange: (event) => updateField("carClass", event.currentTarget.value as CarRentalClass) }, ...CAR_RENTAL_CLASS_OPTIONS.map((option) => createElement("option", { key: option.value, value: option.value }, language === "tr" ? option.trLabel : option.enLabel)))),
        createElement("label", null, language === "tr" ? "Vites" : "Transmission", createElement("select", { style: inputStyle, value: form.transmission, onChange: (event) => updateField("transmission", event.currentTarget.value as CarRentalTransmission) }, ...CAR_RENTAL_TRANSMISSION_OPTIONS.map((option) => createElement("option", { key: option.value, value: option.value }, language === "tr" ? option.trLabel : option.enLabel)))),
        createElement("label", null, language === "tr" ? "Yolcu" : "Passengers", createElement("input", { style: inputStyle, type: "number", min: 1, value: form.passengers, onChange: (event) => updateField("passengers", Number(event.currentTarget.value)) }))
      ),
